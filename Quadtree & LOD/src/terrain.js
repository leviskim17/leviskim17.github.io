...
export const terrain = (function() {
  ...
  class ColourSampler {
    ...
    Get(x, y, z) {
      const m = this._params.biomeGenerator.getHeight(x, z);
      const h = y / 100.0;
  ...
  class TerrainChunk {
    ...
    *_Rebuild() {
      ...
      for (let v of this._plane.geometry.vertices) {
        ...
        colours.push(this._params.colourSampler.Get(v.x + offset.x, v.z, -v.y + offset.y));
  ...
  class TerrainChunkManager {
    ...
    _InitTerrain(params) {
      ...
      const terrainRollup = params.gui.addFolder('Terrain.Mesh');
    ...
    createTerrainChunk(offset, width) {
      const params = {
        ...
        colourSampler: new ColourSampler({biomeGenerator: this._biomes}),
    ...
    updateVisibleChunks_Quadtree() {
      ...
      qt.Insert(this._params.camera.position);

      const children = qt.GetChildren();
      ...
      for (let k in difference) {
        ...
        newTerrainChunks[k] = {
          position: [xp, zp],
          chunk: this.createTerrainChunk(offset, difference[k].dimensions[0]),
        };
      }
    ...
    updateVisibleChunks_FixedGrid() {
      function key(xc, zc) {
        return xc + '/' + zc;
      }

      const [xc, zc] = this.cellIndex(this._params.camera.position);

      const keys = {};

      for (let x = -_FIXED_GRID_SIZE; x <= _FIXED_GRID_SIZE; x++) {
        for (let z = -_FIXED_GRID_SIZE; z <= _FIXED_GRID_SIZE; z++) {
          const key = key(x + xc, z + zc);
          keys[key] = {
            position: [x + xc, z + zc]
          };
        }
